...
def test_newly_initialized_graph(init_g):
    """Test that an empty graph is a dict."""
    assert init_g.graph == {}


def test_newly_initialized_graph_all_edges(init_g):
    """Test that an empty graph has all_edges."""
    assert len(init_g.graph.keys()) == 0
    assert len(init_g.graph.values()) == 0
...
def test_del_node(init_g):
    ...
    init_g.add_node(5)
    init_g.del_node(5)
    assert len(init_g.nodes()) == 0


def test_del_node_key_error(init_g):
